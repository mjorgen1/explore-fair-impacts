...
def immediate_impact_csv(data_path= 'data/results/', group= 0, folders= ['dt','gnb','lgr','gbt']):
    """
    Extracting the Immediate Impact by group out of multiple files , by gettingthe types (FP;TP;FN;TN) calculate it based on the types and save it in an extra csv.
        Args:
            - data_path <str>: data dir of the results
            - group <int>: group indicator (0,1)
            - folders <list>: result folders (classifier)
    ...
    for i,f in enumerate(folders):
        ...
        # calculateing immediate impact: TP-FN
    ...
    df.columns = col_names_df
    print('Group: ',group,'\n DataFrame: \n',df)
    df.to_csv(f'{data_path}/{group}_I.csv')


def delayed_impact_german_csv(data_path= 'data/results/',group = 0, folders= ['dt','gnb','lgr','gbt']):
    """
    Extracting the Delayed Impact by group out of multiple files , by gettingthe types (FP;TP;FN;TN) calculate it based on the types and save it in an extra csv.
    No scores needed
        Args:
            - data_path <str>: data dir of the results
            - group <int>: group indicator (0,1)
    ...
    for i,f in enumerate(folders):
        ...
        col_names_df.append(f'{f.upper()}')
        #delayed impact: TP-FP - FP twice the weight of TP
